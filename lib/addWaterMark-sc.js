  ...
  return new Promise((resolve, reject) => {
    fs.readdir(pathToVideos, (err, files) => {
      ...
      if (listOfVideos.length <= 0) {
        console.error(
          "\x1b[31m",
          "No files with .mp4 extension found inside WATERMARK folder\n".toUpperCase()
        );
      } else {
  ...
  return new Promise((resolve, reject) => {
    new ffmpeg(fileObj.filePath)
      ...
      .on("end", (proc, ffmpegData) => {
        ...
        console.log(
          "\x1b[34m",
          "\nTotal time taken to add watermark: ",
      ...
      .save(
        skipOutput
          ? fileObj.filePath.slice(
              0,
              fileObj.filePath.indexOf(fileObj.fileName)
            ) +
              "w" +
              fileObj.filePath.slice(fileObj.filePath.indexOf(fileObj.fileName))
