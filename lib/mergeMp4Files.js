...
const constructlist = async (pathToVideos, skipOutputFolder) => {
    return new Promise((resolve, reject) => {
        fs.readdir(pathToVideos, (err, files) => {
            ...
            let filesStream = fs.createWriteStream(pathToVideos + '/file.txt');
            errorStream = fs.createWriteStream(pathToVideos + '/error.txt');
            files.filter((file) => {
    ...
    return new Promise((resolve, reject) => {
        ffmpeg(videoFile)
            ...
            .on('error', (err) => {
                ...
                errorStream.end(() => console.log('Errors logged'));
