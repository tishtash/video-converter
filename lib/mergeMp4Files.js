...
const constructlist = async (pathToVideos, skipOutputFolder) => {
  return new Promise((resolve, reject) => {
    fs.readdir(pathToVideos, (err, files) => {
      ...
      filesStream.end(() =>
        console.log("List of file names appended to file.txt!!")
      );
      if (listOfVideos.length <= 0) {
        console.error(
          "\x1b[31m",
          "No files with .mp4 extension found inside MP4 folder"
        );
        process.exit(0);
      } else {
        if (!skipOutputFolder) {
...
const convertVideoScale = async videoFile => {
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(videoFile)
      .outputOptions(["-preset ultrafast", "-codec:a copy"])
      .videoFilters([
        {
          filter: "fade",
      ...
      .on("end", (proc, ffmpegData) => {
        ...
        console.log(
          "\x1b[34m",
          `\nTotal time taken to scale video file: ${videoFile.substring(
            videoFile.lastIndexOf("/") + 1,
            videoFile.lastIndexOf(".")
          )}.mp4: `,
          mpTimeConvert(mpStartTime, mpEndTime)
        );
...
module.exports.init = async (pathToVideos, skipOutputFolder, startTime) => {
  ...
  mergeVideos
    .input(
      `${
        skipOutputFolder
          ? "./videos/AVI/output/temp/file.txt"
          : "./videos/MP4/file.txt"
      }`
    )
    .inputOptions(["-safe 0", "-f concat"])
    .outputOptions(["-preset ultrafast", "-c copy"])
    // .videoFilters([
    //   {
    //     filter: "fade",
    //     options: `t=out:st=${duration - 1}:d=0.5`
    ...
    // }
    // ])
    // ffmpeg -f concat -i ./videos/MP4/file.txt -c copy sample.mp4
    .save(
      skipOutputFolder
        ? "./videos/AVI/output/temp.mp4"
        : pathToVideos + "/output/temp.mp4",
      skipOutputFolder ? "./videos/AVI/output" : pathToVideos + "/output"
    )
    .on("start", command => {
      mpStartTime = moment(new Date());
    ...
    .on("end", (proc, ffmpegData) => {
      ...
      console.log(
        "\x1b[34m",
        "\nTotal time taken to merge: ",
        mpTimeConvert(mpStartTime, mpEndTime)
      );
      console.log(
        "\x1b[34m",
        "\nTotal time taken for entire merge process: ",
        mpTimeConvert(initialTime, mpEndTime)
      );
      console.log(
        "\x1b[34m",
        "\n********************COMPLETED**********************"
